...
namespace AudioAnalyzer.EditorUtilities
{
	...
	public class MatPropertyEditor : InspectorMonoBase<MaterialPropertyAFX>
	{
			...
			for (int i = 0; i < mats.Length; i++)
			{
				...
				for (int j = 0; j < ShaderUtil.GetPropertyCount(mat.shader); j++)
				{
					...
					if (t == ShaderUtil.ShaderPropertyType.Float || t == ShaderUtil.ShaderPropertyType.Vector || t == ShaderUtil.ShaderPropertyType.Range)
					{
						...
						if (!matProps[mat].Contains(propname)) matProps[mat].Add(propname); // for some reason properties come out doubled
					}
				}
			}
		}
